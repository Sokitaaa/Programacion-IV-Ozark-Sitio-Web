...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ======================================================
       5. FILTRO DE PERSONAJES
       ====================================================== */
    const filtros = document.querySelectorAll('.filtro');

    if (filtros.length > 0) {

        const fichas  = document.querySelectorAll('.personaje[data-grupo]');
        const grupos  = document.querySelectorAll('.personajes');
        const titulos = document.querySelectorAll('.titulo-grupo');
        const franja  = document.querySelector('.franja');
        const aviso   = document.getElementById('filtroAviso');
        ...
        filtros.forEach(function (boton) {
            boton.addEventListener('click', function () {
                ...
                fichas.forEach(function (ficha) {
                    const entra = (elegido === 'todos' || ficha.dataset.grupo === elegido);
                    ficha.hidden = !entra;
                    if (entra) visibles++;
                });

                // Si un grupo queda sin tarjetas, se oculta el bloque
                // entero junto con su titulo, si no quedan huecos raros.
                grupos.forEach(function (grupo, i) {
                    const quedan = grupo.querySelectorAll('.personaje:not([hidden])').length;
                    grupo.hidden = (quedan === 0);
                    if (titulos[i]) titulos[i].hidden = (quedan === 0);
                });

                // La franja solo tiene sentido separando los dos grupos
                if (franja) franja.hidden = (elegido !== 'todos');

                if (aviso) {
                    aviso.textContent = (elegido === 'todos')
                        ? ''
                        : visibles + (visibles === 1 ? ' personaje' : ' personajes');
                }
            });
        });
    }
